    ...
    <div className="max-w-6xl mx-auto px-4 py-12 space-y-10">
      ...
      {currentPlan && (
        <div className="text-center">
          <span className="inline-flex items-center gap-2 bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 rounded-full px-4 py-1.5 text-sm font-semibold">
            <Check size={14} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {PLANS.map(plan => {
          ...
          return (
            <div
              key={plan.id}
              className={`relative bg-gray-900 border rounded-3xl p-6 flex flex-col gap-5 transition-all ${active ? 'ring-2 ring-emerald-400/60 border-emerald-500/50' : c.border} ${plan.popular && !active ? 'ring-2 ring-emerald-500/40' : ''}`}
            >
              {active && (
              ...
              {active ? (
                <div className="w-full flex items-center justify-center gap-2 bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 font-bold py-3 rounded-2xl">
                  <Check size={16} />
                  Aktyvus planas
                </div>
              ) : (
