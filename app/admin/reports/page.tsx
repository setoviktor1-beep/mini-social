  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        ...
        <select value={typeFilter} onChange={e => { setTypeFilter(e.target.value); setPage(1) }} className="px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white">
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-gray-50">
            {loading ? (
              ...
              <tr><td colSpan={7} className="px-6 py-12 text-center text-gray-400">No reports found.</td></tr>
            ) : reports.map(report => (
              <>
                <tr key={report.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-3">
                    <button onClick={() => loadTargetContent(report)} className="text-xs font-bold text-blue-600 uppercase hover:underline">
                      {report.target_type}
                    </button>
                  </td>
                  <td className="px-6 py-3 text-gray-600 max-w-xs truncate">{report.reason}</td>
                  <td className="px-6 py-3 text-gray-500 text-xs">@{report.reporter?.username}</td>
                  <td className="px-6 py-3">
                    <select
                  ...
                  <td className="px-6 py-3 text-gray-500 text-xs">{new Date(report.created_at).toLocaleDateString()}</td>
                  <td className="px-6 py-3">
                      ...
                      {report.status === 'open' && (
                        <>
                          {report.target_type !== 'user' && (
                            <>
                              <button onClick={() => handleHideTarget(report)} className="px-2 py-1 text-xs font-medium bg-yellow-50 text-yellow-700 rounded-lg hover:bg-yellow-100">
                                Hide
                              </button>
                              <button onClick={() => handleDeleteTarget(report)} className="px-2 py-1 text-xs font-medium bg-red-50 text-red-700 rounded-lg hover:bg-red-100">
                                Delete
                              </button>
                            </>
                          )}
                          <button onClick={() => handleResolve(report.id)} className="px-2 py-1 text-xs font-medium bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100">
                            Resolve
                          </button>
                          <button onClick={() => handleClose(report.id)} className="px-2 py-1 text-xs font-medium bg-gray-50 text-gray-600 rounded-lg hover:bg-gray-100">
                            Close
                          </button>
                        </>
                      )}
                ...
                {expanded === report.id && targetContent && (
                  <tr key={`${report.id}-content`}>
                    <td colSpan={7} className="px-6 py-4 bg-gray-50 border-l-4 border-blue-400">
