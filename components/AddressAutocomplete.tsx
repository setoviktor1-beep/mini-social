  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      <input
        ...
        className={className || 'w-full border border-gray-200 rounded-xl pl-11 pr-10 py-2.5 text-gray-900 dark:text-white bg-transparent outline-none focus:border-blue-300 focus:ring-1 focus:ring-blue-100 transition-colors placeholder:text-gray-400'}
        autoComplete="off"
      />
      ...
      {open && suggestions.length > 0 && (
        <ul className="absolute z-50 left-0 right-0 mt-1 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg overflow-hidden">
          {suggestions.map(s => (
            <li key={s.placeId}>
              <button
                onMouseDown={() => selectSuggestion(s)}
                className="w-full flex items-start gap-3 px-4 py-3 hover:bg-blue-50 dark:hover:bg-blue-900/20 text-left transition-colors"
              >
